...
class astar:
    ...
    def __init__(self, source, goal, file_name, dump_flag = False):
        ...
        self.fringe = PriorityQueue1() ## Initializing fringe as priority queue which is imported form datastrucute module.
    ...
    def generate_successor(self,node,pos):
        successor = Tools().successor(node,pos,method="A*") ## gives a function call to successor function in Tools class from tools module.
    ...
    def graphSearch(self):
        ...
        heuristic = Tools().heuristicwithcost(self.start_state,self.goal_state)+0 ## calculates the heuristic cost needed for priority list.
        start = node(self.start_state,self.goal_state,{"Start"},0,0,None,empty_tile,heuristic=heuristic) ## creates the node in the required structure as defined in node class in tools.py
